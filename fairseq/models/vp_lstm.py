...
class MLPEncoder(FairseqEncoder):
    ...
    def __init__(
        ...
        if pretrained_embed is None:
            self.embed_tokens = Embedding(self.num_embeddings, embed_dim, self.padding_idx)
        else:
        ...
        # encoder uses source dictionary
        self.excluded_tokens = [self.dictionary.bos(), self.dictionary.eos(), self.dictionary.pad()]
    ...
    def forward(self, src_tokens, src_lengths, **kwargs):
        ...
        # encoder uses source dictionary
        encoder_padding_mask = torch.ones(src_tokens.shape, dtype=bool).cuda()
        for excluded_token in self.excluded_tokens:
            encoder_padding_mask &= (src_tokens != excluded_token)

        mask = torch.ones(src_tokens.shape, dtype=bool).cuda()
        for excluded_token in self.excluded_tokens:
            mask &= (src_tokens != excluded_token)
        lengths = (~mask).sum(dim=1).float().unsqueeze(-1)
        embeddings[mask] = 0
        embeddings = embeddings.sum(dim=1)
        embeddings = embeddings / lengths

        return {
            'encoder_out': (x, final_hiddens, final_cells, embeddings),
...
class MLPDecoder(FairseqIncrementalDecoder):
    ...
    def __init__(
        ...
        padding_idx = dictionary.pad()
        if pretrained_embed is None:
            self.embed_tokens = Embedding(self.num_embeddings, embed_dim, padding_idx)
        else:
            self.embed_tokens = pretrained_embed

        # self.conv2d = nn.Conv2d(1, 1, 2)
        # self.max2d = nn.MaxPool2d(2)

        self.fc_hidden = Linear(self.encoder_output_units + self.embed_dim, self.hidden_size)
        self.norm = LayerNorm(self.hidden_size)
        self.fc_out = Linear(self.hidden_size, self.num_embeddings)

    def forward(self, prev_output_tokens, encoder_out, incremental_state=None, **kwargs):
        encoder_padding_mask = encoder_out['encoder_padding_mask']
        outs, hiddens, cells, embeddings = encoder_out['encoder_out'][:4]

        outs = outs.permute(1, 0, 2)
        outs[encoder_padding_mask] = -float('inf')
        outs = torch.max(outs, dim=1)[0]

        x = torch.cat((outs, embeddings), dim=1)

        # encoder_outs, encoder_hiddens, encoder_cells = encoder_outs[-1], encoder_hiddens[-1], encoder_cells[-1]
        # x = torch.cat((x, hiddens[0], cells[0]), dim=1)
...
@register_model_architecture('vp_lstm', 'vp_lstm')
